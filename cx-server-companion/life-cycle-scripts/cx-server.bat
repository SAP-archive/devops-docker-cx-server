...
IF "%DEVELOPER_MODE%"=="" docker pull ppiper/cx-server-companion:teched

(
    docker run --rm -it --workdir /cx-server/mount --volume //var/run/docker.sock:/var/run/docker.sock --mount source="%CD%",target=/cx-server/mount,type=bind --env DEVELOPER_MODE --env host_os=windows --env cx_server_path="%CD%" ppiper/cx-server-companion:teched /cx-server/cx-server-companion.sh %~1 %~2
